...
export const launchGame = async ({ placeId }) => {
  const result = await request('GET', `/game/get-join-script?placeId=${encodeURIComponent(placeId)}`);
...
export const getServers = ({ placeId, offset }) => {
  return request('GET', `/games/getgameinstancesjson?placeId=${placeId}&startIndex=${offset}`).then(d => d.data);
}
