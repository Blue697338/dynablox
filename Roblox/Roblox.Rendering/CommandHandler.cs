...
namespace Roblox.Rendering
{
    public static class CommandHandler
    {
        ...
        {
            Console.WriteLine("[CommandHandler] Configure called with baseUrl={0}", baseUrl);
            var url = new Uri(baseUrl + "?key=" + HttpUtility.UrlEncode(authorization));
            wsUrl = url;
            Console.WriteLine("[CommandHandler] WebSocket URL set to: {0}", wsUrl);

            Task.Run(async () =>
            {
                Console.WriteLine("[CommandHandler] Starting ConnectionManager task");
        ...
        private static async Task ConnectionManager()
        {
            while (true)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine("[CommandHandler] ws connection is in state {0}, attempting to reconnect to {1}", ws.State, wsUrl);
                        mux.WaitOne();
                        ws = new ClientWebSocket();
                        mux.ReleaseMutex();
                        try
                        {
                            ...
                            Console.WriteLine("[CommandHandler] Successfully connected to WebSocket at {0}", wsUrl);
                        }
                        catch (Exception connectEx)
                        {
                            Console.WriteLine("[CommandHandler] Failed to connect to WebSocket: {0}", connectEx.Message);
                            throw;
                        }
                    }
                ...
                {
                    Console.WriteLine("[CommandHandler] ConnectionManager got error: {0}. Retrying in 5 seconds...", e.Message);
        ...
        private static async Task<RenderResponse<Stream>> SendCommand(string command, IEnumerable<dynamic> arguments, CancellationToken? cancellationToken)
        {
            ...
            Console.WriteLine("[CommandHandler] SendCommand: {0} (id={1})", command, id);
            
            // Setup listener
            mux.WaitOne();
            resultListeners[id] = stream =>
            {
                Console.WriteLine("[CommandHandler] SendCommand() received response for id={0}", id);
                lock (responseMutex)
                {
            ...
            {
                Console.WriteLine("[CommandHandler] WebSocket not open (state={0}), waiting...", ws?.State);
            ...
            Console.WriteLine("[CommandHandler] Sending command {0} to WebSocket", command);
