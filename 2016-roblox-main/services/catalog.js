...
export const getComments = async ({ assetId, offset }) => {
  return request('GET', `/comments/get-json?assetId=${assetId}&startIndex=${offset}&thumbnailWidth=100&thumbnailHeight=100&thumbnailFormat=PNG&cachebuster=${Math.random()}`).then(d => d.data);
}

export const createComment = async ({ assetId, comment }) => {
  let result = await request('POST', `/comments/post`, {
    text: comment,
    assetId: assetId,
  });
...
export const addOrRemoveFromCollections = ({ assetId, addToProfile }) => {
  return request('POST', `/asset/toggle-profile`, {
    assetId,
    addToProfile,
  })
}
...
export const deleteFromInventory = ({ assetId }) => {
  return request ('POST', `/apisite/inventory/v1/delete-from-inventory`, {
    assetId: assetId
  })
}
