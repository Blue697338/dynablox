...
export default class CommandHandler extends StdExceptions {
	...
	private onStartup() {
		...
		try {
			// Skip RCC cleanup on Windows - the shell script doesn't work on Windows
			if (process.platform === 'win32') {
				console.log('[info] skipping RCC cleanup on Windows');
				return;
			}
	...
	private async startRcc(game: IGameEntry): Promise<void> {
		// On Windows, RCCService doesn't support dynamic port configuration.
		// Just check if it's already running on the configured port and use it.
		if (game.rccReference || ( await this.isRccReady(rccPort))) {
			console.log('[info] rcc already running on port', rccPort);
...
<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">
  <soap:Body>
	...
	private async runJobQueueTask(rcc: IGameEntry, job: IQueueEntry) {
		...
		console.log('[info] [rcc] starting job', job.jobId, 'on port', rcc.rccReference.port);
		// send request
		try {
			const result = await axiosClient.request({
				method: 'POST',
			...
			console.log('[info] [rcc] job request sent successfully', job.jobId);
		} catch (e) {
			console.error('[error] [rcc] job request failed', job.jobId, e.message);
			throw e;
		}
		// wait until over
		console.log('[info] [rcc] awaiting result for job', job.jobId);
		await awaitResult(job.jobId);
		console.log('[info] [rcc] job completed', job.jobId);
	}
