...
public class AdminApiController : ControllerBase
{
        ...
        int offset = 0, string? query = null)
    {
        ...
        return new
        {
            data = all.Select(c =>
            {
                c.status = c.status != null ? ((AccountStatus)c.status).ToString() : AccountStatus.Ok.ToString();
                c.trade_filter = c.trade_filter != null ? ((TradeQualityFilter)c.trade_filter).ToString() : TradeQualityFilter.None.ToString();
                c.inventory_privacy = c.inventory_privacy != null ? ((GeneralPrivacy)c.inventory_privacy).ToString() : GeneralPrivacy.All.ToString();
                c.trade_privacy = c.trade_privacy != null ? ((GeneralPrivacy)c.trade_privacy).ToString() : GeneralPrivacy.All.ToString();
                c.private_message_privacy = c.private_message_privacy != null ? ((GeneralPrivacy)c.private_message_privacy).ToString() : GeneralPrivacy.All.ToString();
                c.gender = c.gender != null ? ((Gender)c.gender).ToString() : Gender.Unknown.ToString();
    ...
    public async Task<dynamic> GetUserInfoDetailed([FromQuery] long userId)
    {
        ...
        result.theme = result.theme != null ? ((ThemeTypes) result.theme).ToString() : ThemeTypes.Light.ToString();
        result.status = result.status != null ? ((AccountStatus)result.status).ToString() : AccountStatus.Ok.ToString();
        result.trade_filter = result.trade_filter != null ? ((TradeQualityFilter)result.trade_filter).ToString() : TradeQualityFilter.None.ToString();
        result.inventory_privacy = result.inventory_privacy != null ? ((GeneralPrivacy)result.inventory_privacy).ToString() : GeneralPrivacy.All.ToString();
        result.trade_privacy = result.trade_privacy != null ? ((GeneralPrivacy)result.trade_privacy).ToString() : GeneralPrivacy.All.ToString();
        result.private_message_privacy = result.private_message_privacy != null ? ((GeneralPrivacy)result.private_message_privacy).ToString() : GeneralPrivacy.All.ToString();
        result.gender = result.gender != null ? ((Gender)result.gender).ToString() : Gender.Unknown.ToString();
...
-The Roblox Team"
    ...
    [HttpPost("asset/render"), StaffFilter(Access.SetAssetProduct)]
    public async Task RenderAsset([Required, FromBody] RenderAssetRequest request)
    {
        var assetInfo = await services.assets.GetAssetCatalogInfo(request.assetId);
        await services.assets.RenderAssetAsync(request.assetId, assetInfo.assetType);
    }
    ...
    public async Task<dynamic> CopyAssetFromRoblox([Required, FromBody] CopyAssetRequest request)
    {
        ...
        // Skip validation for admin imports - admins are trusted
        ...
        Writer.Info(LogGroup.AdminApi, "Asset created from Roblox. assetId={0}, robloxAssetId={1}, name={2}", assetDetails.assetId, request.assetId, details.Name);
        
        if (assetDetails.assetId == 0)
            throw new StaffException("Asset creation failed - assetId is 0");
