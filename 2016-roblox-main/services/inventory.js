...
export const getInventory = ({ userId, limit, cursor, assetTypeId }) => {
  return request('GET', `/users/inventory/list-json?userId=${userId}&assetTypeId=${assetTypeId}&cursor=${encodeURIComponent(cursor || '')}&itemsPerPage=${limit}`).then(d => d.data);
}

export const getFavorites = ({ userId, limit, cursor, assetTypeId }) => {
  return request('GET', `/users/favorites/list-json?userId=${userId}&assetTypeId=${assetTypeId}&pageNumber=${cursor || 1}&itemsPerPage=${limit}`).then(d => d.data).then(d => {
...
export const getCollections = ({ userId }) => {
  return request('GET', `/users/profile/robloxcollections-json?userId=${userId}`).then(d => d.data.CollectionsItems)
}
