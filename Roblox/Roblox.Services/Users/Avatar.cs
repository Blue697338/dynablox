...
public class AvatarService : ServiceBase, IService
{
    ...
    public async Task<ColorEntry> GetAvatarColors(long userId)
    {
        ...
        if (existingAvatar == null)
        {
            // Return default colors if user doesn't have an avatar
            // Using valid color IDs: 364 = Dark taupe (a valid default color)
    ...
    {
        Console.WriteLine("[RedrawAvatar] Starting for userId={0}, forceRedraw={1}", userId, forceRedraw);
        // required services
        using var assets = ServiceProvider.GetOrCreate<AssetsService>();
        ...
        Console.WriteLine("[RedrawAvatar] Got assetIds={0}, colors={1}", assetIds.Count, colors != null);
        Console.WriteLine("[RedrawAvatar] Colors: headColorId={0}, torsoColorId={1}, leftArmColorId={2}, rightArmColorId={3}, leftLegColorId={4}, rightLegColorId={5}", 
            colors.headColorId, colors.torsoColorId, colors.leftArmColorId, colors.rightArmColorId, colors.leftLegColorId, colors.rightLegColorId);
        ...
        Console.WriteLine("[RedrawAvatar] Updated avatar hash={0}", avatarHash);
        // Get our image urls
        ...
        if (!forceRedraw)
        {
            ...
            {
                Console.WriteLine("[RedrawAvatar] Files already exist, skipping render");
        ...
        Console.WriteLine("[RedrawAvatar] Starting render for userId={0}", userId);
        ...
        Console.WriteLine("[RedrawAvatar] Sending render requests to game-server for userId={0}", userId);
        ...
        Console.WriteLine("[RedrawAvatar] Render requests completed for userId={0}", userId);
        ...
        Console.WriteLine("[RedrawAvatar] Files written for userId={0}", userId);
        // Finally, update the avatar thumbnail
        await UpdateUserAvatarImages(userId, headshotUrl, thumbnailUrl);
        Console.WriteLine("[RedrawAvatar] Completed for userId={0}", userId);
    }
