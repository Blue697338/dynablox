...
public class AccountInformationService : ServiceBase, IService
{
    ...
    public async Task<InventoryPrivacy> GetUserInventoryPrivacy(long userId)
    {
        ...
        return settings?.inventoryPrivacy ?? InventoryPrivacy.NoOne;
    }
    ...
    {
        var settings = await GetUserSettings(userId);
        return settings?.tradePrivacy ?? GeneralPrivacy.All;
    }
    ...
    {
        var settings = await GetUserSettings(userId);
        return settings?.tradeFilter ?? TradeQualityFilter.None;
    }
