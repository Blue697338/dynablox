...
app.post('/api/upload-thumbnail-v1', async (req, res, next) => {
		...
		if (req.body) {
			...
			if (js['accessKey'] === conf.authorization) {
				...
				if (callbacks) {
					...
					// Forward the thumbnail to the backend
					try {
						console.log('[info] forwarding thumbnail to backend');
						const backendResponse = await axiosClient.post(
							conf.baseUrl + '/apisite/avatar/v1/upload-thumbnail-v1',
							{
								userId: String(js['userId']),
								thumbnail: js['thumbnail'],
								isHeadshot: js['type'] === 'PlayerHeadshot'
							}
						);
						console.log('[info] backend response:', backendResponse.data);
					} catch (e: any) {
						console.error('[error] failed to forward thumbnail to backend:', e.message);
						if (e.response) {
							console.error('[error] backend response status:', e.response.status);
							console.error('[error] backend response data:', e.response.data);
						}
						// Don't fail the response - the thumbnail was still received by the game-server
					}
